...
#
# Writes: dist/r-type-delta-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix r-type-delta \
  --exe-name R_Type_Delta_Recompiled \
  --display-name "R-Type Delta Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "R-Type Delta (USA).cue" \
